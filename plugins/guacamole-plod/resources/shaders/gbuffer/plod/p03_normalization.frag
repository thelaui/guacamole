...
layout(binding=4) uniform sampler2D p02_position_texture;
    ...
    */
    if(   ( (bit_pattern & 0xD6) == 0xD6)
       ...
       || ( (bit_pattern & 0x2F) == 0x2F)
        ...
        vec3 final_neighbours_position = vec3(0.0, 0.0, 0.0);
    ...
    for(unsigned int bit_index = 0; bit_index < 8; ++bit_index) {
      if( (bit_pattern & (0x1 << bit_index) ) != 0x0 ) {
        ...
        vec2 current_neighbours_weight_and_depth = texelFetch(p02_weight_and_depth_texture, current_fragment_pos + lookup_index_xy,0 ).xy;
            ...
            final_neighbours_position +=  (texelFetch(p02_position_texture, current_fragment_pos + lookup_index_xy,0 ).xyz) /  current_neighbours_weight_and_depth.x;
    ...
    final_neighbours_position /= num_accumulated_neighbours;
    ...
    gua_world_position = final_neighbours_position;
    ...
    vec3 accumulated_position = texelFetch(p02_position_texture, current_fragment_pos, 0).rgb;
